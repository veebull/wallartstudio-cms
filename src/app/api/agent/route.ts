...
export async function POST(req: NextRequest) {
  ...
  const [tmpl] = await db
    ...
    .where(eq(articleTemplates.id, templateId));
  if (!tmpl)
  ...
  setImmediate(() =>
    runAgentJob(job.id, tmpl.blocks as Block[], pendingCities, cfg),
  );
      ...
      try {
        ...
        await db
          ...
          .values({
            ...
            templateId: (
              await db
                .select({ templateId: agentJobs.templateId })
                .from(agentJobs)
                .where(eq(agentJobs.id, jobId))
            )[0]?.templateId,
